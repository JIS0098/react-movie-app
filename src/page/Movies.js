            ...
            <DropdownButton id="dropdown-item-button" title="Sort">
              ...
              <DropdownButton id="dropdown-item-button" title="Sort By">
                ...
                <Dropdown.Item onClick={() => {
                  let copy = [...list]; copy.sort((a, b) => (b.popularity - a.popularity)); setList(copy);
                  popularMovies.results.sort((a, b) => (b.popularity - a.popularity))
                }} as="button">Popularity(Desc)</Dropdown.Item>
                <Dropdown.Item onClick={() => {
                  let copy = [...list]; copy.sort((a, b) => (a.popularity - b.popularity)); setList(copy);
                  popularMovies.results.sort((a, b) => (a.popularity - b.popularity))
                }} as="button">Popularity(Asc)</Dropdown.Item>
                <Dropdown.Item onClick={() => {
                  let copy = [...list]; copy.sort((a, b) => (b.release_date - a.release_date)); setList(copy);
                  popularMovies.results.sort((a, b) => (b.release_date - a.release_date))
                }} as="button">Release Day(Desc)</Dropdown.Item>
                <Dropdown.Item onClick={() => {
                  let copy = [...list]; copy.sort((a, b) => (a.release_date - b.release_date)); setList(copy);
                  popularMovies.results.sort((a, b) => (a.release_date - b.release_date))
                }} as="button">Release Day(Asc)</Dropdown.Item>
                <Dropdown.Item onClick={() => {
                  let copy = [...list]; copy.sort((a, b) => (b.vote_average - a.vote_average)); setList(copy);
                  popularMovies.results.sort((a, b) => (b.vote_average - a.vote_average))
                }} as="button">Vote(Desc)</Dropdown.Item>
                <Dropdown.Item onClick={() => {
                  let copy = [...list]; copy.sort((a, b) => (a.vote_average - b.vote_average)); setList(copy);
                  popularMovies.results.sort((a, b) => (a.vote_average - b.vote_average))
                }} as="button">Vote(Asc)</Dropdown.Item>
                <Dropdown.Item onClick={() => {
                  let copy = [...list]; copy.sort((a, b) => (b.popularity - a.popularity)); setList(copy);
                  popularMovies.results.sort((a, b) => (a.popularity - b.popularity))
                }} as="button">Revenue(Desc)</Dropdown.Item>
                <Dropdown.Item onClick={() => {
                  let copy = [...list]; copy.sort((a, b) => (b.popularity - a.popularity)); setList(copy);
                  popularMovies.results.sort((a, b) => (a.popularity - b.popularity))
                }} as="button">Revenue(Asc)</Dropdown.Item>
              </DropdownButton>
            </DropdownButton>
            <div>Filter</div>
          </div>
